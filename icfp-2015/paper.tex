...
% TODO: Cleanup "strucutred" vs "structural"
% TODO: Call it "relativized safety" or "relativized soundness" consistently
...
the relativized soundness property in Section~\ref{sec:safety}.
...
\inference
  {~}
  {\Gamma \vdash s : \ident{StructVal}}
\qquad  
\inference
  {~}
  {\Gamma \vdash n : \ident{int}}
\end{equation*}

\begin{equation*}
\inference
  {\Gamma \vdash e : C & \kvd{type}~C(\overline{x:\tau}) = ..\;\kvd{member}~N_i : \tau_i = e_i\;..}
  {\Gamma \vdash e.N_i:\tau_i}
\end{equation*}

\begin{equation*}
\inference
  {\Gamma \vdash e_i : \tau_i & \kvd{type}~C(x_1:\tau_1, \ldots, x_n:\tau_n) = \ldots}
  {\Gamma \vdash \kvd{new}~C(e_1, \ldots, e_n):C}
\end{equation*}
...
Selected type checking rules for Featherweight F\# are shown in Figure~\ref{fig:ff-typecheck}.
...
Given a rule from the type provider, when we have a structural value that 

Given $\semalt{s} :> \semalt{s'}$ and $\tau, e, L = \sem{\semalt{s}}_{s'}$ then
either $e \reduce v$ or $\tau=C$ and for all fields $e.N \reduce v$.
\end{lemma}
\begin{proof}
?
\end{proof}
...
\end{lemma}
\begin{proof}
by induction over the reduction $\reduce$ (this should be fairly standard)
\end{proof}
...
\end{lemma}
\begin{proof}
by induction over the typing of $e$ (this is standard bit) 
also using the observation that all provided classes are well-typed
(by handwaving). The interesting case is when we do anything on StructVal, in which case
we need:

All access on StructVals happens in the code generated by type provider
and we can use the lemma...
\end{proof}
...
Then for all new inputs $s'$ such that $\exists i.(\semalt{s_i} :> \semalt{s'})$, and for any $e$ 
such that $x:\ident{StructVal} \vdash e:\tau$ using class definitions $L$, it is the case that 
$e[x\leftarrow s'] \reduce^{*} v$ and $\vdash v : \tau$.
\end{theorem}
\begin{proof}
