...
% TODO: Adapt XML example 
% TODO: Switch from sum types to variant types
% TODO: Discuss 'stability' of the inference (minor change in schema does not cause dramatic cahnges)
% TODO: Say that any variant is a subtype of any other variant 
% TODO: Say that anything is a subtype of variant
% TODO: Say that variant is a last-resort thing
% TODO: Double check that bottom/top are used correctly
% TODO: Explain (even more) that there is no least upper bound, which is why Theorem 3 is good enough
% TODO:  -- but maybe we can have lub w.r.t variants? or something?
% TODO: Avoid "no other rule applies" in the formal system
...
\begin{array}{rcl}
 ...
 \\[0.6em] 
       \sigma &\narrow{=}& ~\hat{\sigma}~ \lsep \kvd{option}\langl \hat{\sigma} \rangl \lsep [\sigma] \\[0.1em]
              &\narrow{|}& \kvd{var}\langl\sigma_1, \ldots, \sigma_n\rangl \lsep ~\top~ \lsep \kvd{null}
\end{array}
\end{equation*}
%
...
Finally, a variant type \kvd{var} is annotated with the statically known types it may represent based
on the sample document. As discussed earlier (Section~\ref{sec:providers-xml}), the variant type forces
the user to handle the case when a value is of a different type than the ones statically known. For
this reason, the variant type also implicitly permits the \kvd{null} value. 
...
\noindent\boxed{\emph{Variant types}}
\begin{align}
\label{eq:sub-var-bot}\tag{U1}
\kvd{var}\langl \sigma_1, \ldots, \sigma_n\rangl :>&~ \sigma \\
\label{eq:sub-var-eq}\tag{U2}
\kvd{var}\langl \sigma_1, \ldots, \sigma_n \rangl :>&~ \kvd{var}\langl \sigma_1', \ldots, \sigma_n' \rangl
\end{align}
...
\item The \ident{bit} type is a subtype of both \ident{int} and \ident{bool} (\ref{eq:sub-prim}). 
  ...
  $0,1,2$ becomes \ident{int} (for a sample $0,1,2, \kvd{true}$ we would need a variant type).
...
\item There is a top type (\ref{eq:sub-top}) and variants can be seen as the bottom because
  any type $\sigma$ is a subtype of a variant (\ref{eq:sub-var-bot}). There is no \emph{single} 
  bottom type -- there is a range of distinct variant types, but any variant is a subtype
  of any other variant (\ref{eq:sub-var-eq}).
...
\noindent
If we were proposing a type system for type-checking programs, the use of multiple variant types
(which all are subtypes of each other) would not add any value and we could introduce a single 
bottom type instead. However, our purpose is different -- we aim to make reading structured data
easier and, as shown in Section~\ref{sec:providers-xml}, the types forming a variant provide a
useful information about the sample document.

Another important rule is (\ref{eq:sub-record4}), which allows subtype to have fewer record 
elements. In some cases, this allows us to infer records rather than variants. For example, given 
...
which is also a record. This is more precise and more usable type than another common supertype
...
$\{\ident{?a}\!:\!\ident{int}, \ident{?b}\!:\!\ident{bool}\}$ and 
$\kvd{var}\langl\{\ident{a}\!:\!\ident{int}\}, \{\ident{b}:\ident{bool}\}\rangl$

NOT TRUE
which are unrelated.

 Our inference algorithm always prefers records over unions (Theorem~\ref{thm:no-unions}). 
