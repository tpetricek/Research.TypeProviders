...
When extracting data from similar XML documents, we often need to get nodes of a specific name (all
items) or extract the content of a node (feed title). The type provider we present is optimized for 
...
Our type inference algorithm for structured data formats is based on a subtyping relation. When 
inferring the type of a document, we infer the most specific types of individual values (CSV rows,
JSON or XML nodes) and then find a common supertype of values in the given sample.
...
\begin{array}{rcl}
 ...
 \\[0.6em] 
       \sigma &\narrow{=}& ~\hat{\sigma}~ \lsep \hat{\sigma}\;\,\kvd{option}\\[0.2em]
                &\narrow{|}& [\sigma] \lsep \sigma_1 + \ldots + \sigma_n \\[0.2em]
                &\narrow{|}& ~\top~ \lsep \kvd{null}
\end{array}
\end{equation*}
%
...
with same name, but different fields and between union types. This following definition makes
this precise.
...
\hspace{-1em}\sigma_1 + \ldots + \sigma_n :> \sigma_{\pi(1)} + \ldots + \sigma_{\pi(n)} \quad &(\textnormal{for a permutation}~\pi) \\
...
\end{array} \qquad &(\textnormal{for a permutation}~\pi)
\end{align}
\vspace{-0.6em}
\begin{align}
...
\item The \ident{bit} type is a subtype of both \ident{int} and \ident{bool} (\ref{eq:sub-prim}). 
  Thus, a sample $0,1$ has a type \ident{bit}, but $0,1,\kvd{true}$ is \ident{bool} and
...
\caption{\small{Inference judgements that define the common supertype relation}}
...
\end{equation*}
%
The tag of a type is obtained using a helper function $\tytagof{(-)} : \sigma \rightarrow \tytag$:
%
\begin{equation*}
\begin{array}{rcl}
 \tytagof(\ident{string}) &\narrow{=}& \ident{string}\\
...
\end{equation*}
%
The remaining rules are mostly straightforward. For collections, we find the common supertype of 
...
The common supertype relation finds a single supertype (among multiple possible candidates).
...
\end{theorem}
\begin{proof}
By induction over the common supertype derivation using $\vdash$.
\end{proof}
...
\hspace{3em}
\begin{array}{l}
 \ident{asBool}(i) \reduce \kvd{true}  \quad(i = 1)\\
 \ident{asBool}(d) \reduce \kvd{true}  \quad(d = 1.0)\\
 \ident{asBool}(f) \reduce \kvd{true}  \quad(f = 1.0)\\
\end{array}
\qquad
\begin{array}{l}
 \ident{asBool}(i) \reduce \kvd{false} \quad(i = 0)\\
 \ident{asBool}(d) \reduce \kvd{false} \quad(d = 0.0)\\
 \ident{asBool}(f) \reduce \kvd{false} \quad(f = 0.0)\\
\end{array}
\qquad
\begin{array}{l}
 \ident{asBool}(s) \reduce \kvd{true}  \quad(s=\str{true} \vee s=\str{1}) \\
 \ident{asBool}(s) \reduce \kvd{false} \quad(s=\str{false} \vee s=\str{0}) \\
 \ident{asBool}(b) \reduce b\quad(\textnormal{when}~b=\kvd{true}\vee b=\kvd{false})
\end{array}
\end{equation*}
%
\begin{equation*}
...
\begin{array}{l}
 ...
 \ident{asInt}(s) \,\reduce i\quad(s~\text{represents}~i) \\
...
\begin{array}{l}
 ...
 \ident{asDec}(s) \,\reduce d\quad(s~\text{represents}~d) \\
...
\begin{array}{l}
 ...
 \ident{asFloat}(s) \,\reduce f\quad(s~\text{represents}~f) \\
 \ident{asFloat}(\kvd{true}) \reduce 1.0\\
 \ident{asFloat}(\kvd{false}) \reduce 0.0\\
\end{array}
\quad
\begin{array}{l}
 \ident{asStr}(i) \,\reduce s\quad(s~\text{represents}~i)\\
 \ident{asStr}(d) \reduce s\quad(s~\text{represents}~d)\\
 \ident{asStr}(f) \reduce s\quad(s~\text{represents}~f)\\
 \ident{asStr}(s) \,\reduce s\\
...
\begin{array}{l}
 ...
 \ident{getField}(\epsilon,\nu_i,\{\ldots, \nu_i=s_i, \ldots\}) \reduce s_i\\
\end{array}
\end{equation*}
}}
\caption{\small{Reduction rules for conversion functions}}
\label{fig:op-conversions}
\end{figure*}

% TODO: We'll need more things here for working with options 
% Also, 'getField' should return 'null' if the field does not exist
% (which we will presumably handle nicely thanks to handling of options)
...
uses a different implementation for each format (reusing existing libraries).}. 
A structural value are first-order and can be one of the primitive values 
(Boolean, string, integer, decimal and float), a missing value (\kvd{null}), a record 
(named or unnamed) and a collection:
%
\begin{equation*}
\begin{array}{lcl}
 s  &\narrow{=}& i \lsep d \lsep f \lsep s \lsep \kvd{true} \lsep \kvd{false} \lsep \kvd{null} \\[0.1em]
...
not \emph{necessary} conditions. This means that a type provided based on sample is guaranteed to work 
...
Types are primitive types or a class or a StructVal, which represents different structured values

L
\begin{equation*}
\begin{array}{l}
 \tau = \ident{int}\lsep\ident{decimal}\lsep\ident{float}\lsep\ident{bool}\lsep\ident{string}\lsep \ident{StructVal} \lsep C \\[0.2em]
 L = \kvd{type}~C(\overline{f:\tau}) = \overline{M} \\[0.2em]
 M = \kvd{member}~N:\tau=e \\[0.2em]
 v = ...
\end{array}
\end{equation*}

.. todo -- evaluation goes here too

\subsection{Type providers}
\label{sec:formal-tp}

How we turn inferred structural type into F\# type
