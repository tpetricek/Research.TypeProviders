...
\item We introduce the notion of relativized type safety and show how it holds for the code
  written using our type providers (Section~\ref{sec:safety}). Although we focus on our specific
  case, this demonstrates a new way of thinking about ML-style type safety that is needed
...
More details about the full type erasure mechanism can be found in the paper on F\# type 
providers [?]. We present a simplified model when discussing safety in Section~\ref{sec:safety}.
...
precise in Section~\ref{sec:safety} when discussing safety.
...
% - Featherweight F\#
% - Generating FF\# from inferred types
