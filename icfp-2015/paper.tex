...
look at the main theorem of the paper. It statest that, for any input (which is a subtype of any of the
samples) and any expression $e$, a well-typed program that uses the provided types does not ``go wrong''.

Using the standard approach to syntactic type safety [?], we prove the type preservation 
...
Sample is captured at compile time. It can come from the web or not (cache locally if you want
but from the web has nice implications).

We dont know what to do about schema change at runtime
(for some data sources, we could do more clever things though)
...
Statistical metric makes the safety even messier (probabilistic!)

Write API

~

F\# and Agda type providers

Kathleen Fisher's work

JSON inference stuff
