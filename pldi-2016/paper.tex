\documentclass[10pt,clearpagebib]{sigplanconf}
...
One of the design motivations mentioned in Section~\ref{sec:providers} is that the inference
mechanism is predictable and stable. Informally this means that ...

In particular, we do 
not use probabilistic methods where adding an additional sample could completely change the 
shape of the type.

For the purpose of this section, assume conversion function \ident{int} and an error value
\ident{error} that propagates, i.e. $C[\ident{error}]\reduce\ident{error}$

\begin{theorem}[Stability of inference]

Given set of samples $d_1, \ldots, d_n$

And $\tau_1, e_1, L_1 = \sem{\semalt{d_1, \ldots, d_n}}$

And some code $L_1; x:\tau_1\vdash e : \tau$

~

Now, add $d_{n+1}$ and let
 $\tau_2, e_2, L_2 = \sem{\semalt{d_1, \ldots, d_n, d_{n+1}}}$

There is $e'$ 

such that $L_2; x:\tau_2\vdash e' : \tau$

and if for some $d$ it is the case that $e[x\leftarrow e_1~d] \reduce v$ then
...
\begin{itemize}
\item[--]
$e$ to $\kvd{match}~e~\kvd{with}~\ident{Some}(v) \rightarrow v~|~\ident{None} \rightarrow \ident{wrong}$
\item[--]
$e$ to $e.\ident{Member}$ where $\ident{Member} = \tytagof(\sigma)$ for some $\sigma$
\item[--]
$e$ to $\ident{int}(e)$
\end{itemize}
\end{theorem}
...
Primitive type - can turn into float or into any with the primitive 
type exposed as one of the members (because it has unique tag), possibly turned to float

Record - introduce new fields, change types of fields - by induction
or not record, in which case it becomes member via any

List - element type change or becomes member of any

Any - ok

nullable - stays nullable, or if they're not compatible, part of any
...
on type providers, including Jomo Fisher, Keith Battocchi and Kenji Takeda.
