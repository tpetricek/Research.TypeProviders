...
The JSON format used in \S\ref{sec:introduction} is a populardata exchange format based on 
...
would throw an exception or skip values in incorrect format. Even if we defined helper functions, 
...
the traditional ML type safety is discussed in \S\ref{sec:safety-discuss}.
...
is extremely useful when reading data from external data sources, because the sample document only
...
a variant is not used if there is another option (Corollary~\ref{thm:no-unions}).
...
in real-world inputs (missing data, inconsistent encoding of primitive values and heterogeneous 
