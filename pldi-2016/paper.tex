...
\begin{equation*}
\begin{array}{rcl}
 \hat{\sigma} &\narrow{=}& \nu \; \{ \nu_1 : \sigma_1, \ldots, \nu_n : \sigma_n, \addopt{\rho_i}  \} \\[0.1em]
...
During inference we use row-variables $\rho_i$ to represent the flexibility
implied by our interpretation of records in samples. 
When a record {\small\ttfamily \{ x : 3 \}} occurs in a sample,
it may be combined in a list with other samples that contain more fields. The
overall shape inferred must account for the potential that any other fields are not
present in the first sample. The solutions of $\rho_i$ variables is 
discussed in \S\ref{sec:inference}. We assume substitution and row-unification is
defined for these in the usual way , except that as substitutions are made
the $\addopt{-}$ operator from Figure~\ref{fig:subtyping-cst} is applied to all types
in the vector. After inference any remaining $\rho_i$ variables are ignored (assumed empty).
...
data $d_1, \ldots, d_n$ into a shape $\sigma$. We assume each record in each $d_i$ is marked with a fresh row inference
variable $\rho_i$.  We then choose a minimal solution $\theta$ for these row variables
in order to equate the record 
field labels and satisfy the side conditions in rule (record) when multiple record samples are merged.
This minimization is implemented via row variable unification, we omit the details here.
