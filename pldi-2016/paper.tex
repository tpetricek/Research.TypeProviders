...
\ident{csh}(\sigma_1, \sigma_2) &=& \kvd{any} & (\emph{any})\\
\end{array}
\end{equation*}
%
\begin{equation*}
...
respective fields. The field labels of two record types can always be made to match, through the 
shape inference from sample (\S\ref{sec:formal-inferval}), \emph{before} finding a common preferred 
shape using $\ident{csh}$.
...
The \ident{csh} function finds the unique least upper bound of the partially ordered 
...
\begin{proof}
By induction over the definition of $\ident{csh}$. Note that the algorithm only infers the top shape 
...
We then choose a ground, minimal, global substitution $\theta$ for row variables. The substitution
is minimal in the sense that it introduces as few new fields as possible, but no additional fields, 
and the newly introduced fields have the most specific shape. Because $\rho_i$ variables represent 
potentially missing fields, the $\addopt{-}$ operator from Figure~\ref{fig:subtyping-cst} is applied 
to all types in the vector. 
...
Figure~\ref{fig:op-conversions}. The typing is shown in Figure~\ref{fig:ff-typecheck} and is 
...
\\[1.75em]
\inference
  { L; \Gamma \vdash e : \tau & \tau \in \{ \ident{int}, \ident{float} \} }
...
\inference
  { L; \Gamma \vdash e : \ident{Data} \\
    (\sigma,\tau)\in\{ (\ident{int},\ident{int}), (\ident{string},\ident{string}), (\ident{bool},\ident{bool}) \} }
  { L; \Gamma \vdash \ident{convPrim}(\sigma, e) : \tau }
\\[1.75em]
\inference
  { L; \Gamma \vdash e_1 : \ident{Data} & L; \Gamma \vdash e_2 : \ident{Data} \rightarrow \tau }
